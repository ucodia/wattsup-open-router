      ...
      <div className="grid gap-8 md:grid-cols-2">
        <ResponsiveContainer width="100%" height={300}>
          <PieChart>
            ...
            <Legend formatter={(value, entry) => `${entry.payload.name}: ${formatNumber(entry.payload.tokens)}`} />
          </PieChart>
        </ResponsiveContainer>
    ...
    <div className="mb-6 flex items-center gap-2">
      {options.map((opt) => (
        <Button
          ...
          className={period === opt ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}
        >
          {PERIOD_LABELS[opt]}
        </Button>
      ))}
